...
def register():
    ...
    if register_form.validate_on_submit():
        lozinka = register_form.password.data
        hashed_and_salted_password = generate_password_hash(register_form.password.data, method='pbkdf2:sha256', salt_length=8)
        new_user = User(email=register_form.email.data, password=hashed_and_salted_password, name=register_form.name.data)
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        if not user:
            flash('This email is not in the database. Please try again')
